...
const SUBSCRIPTION_TIERS = {
  starter:    { price_usd: 20.0, calls_per_day: 50000, label: 'Starter' },
  pro:        { price_usd: 99.0, calls_per_day: 500000, label: 'Pro' },
...
async function emitSpectralReceipt({ event_type, did, amount_usd, tool_name, tx_hash, metadata }) {
  ...
  try {
    ...
    console.warn('[flag] receipt emit failed (non-fatal):', _.message || _);
  }
}
...
app.post('/v1/subscription', async (req, res) => {
  ...
  if (tier !== 'enterprise' && !tx_hash) {
    return res.status(402).json({
      error: 'payment_required',
      x402: {
        type: 'x402', version: '1', kind: 'subscription_flag',
  ...
  await emitSpectralReceipt({
    ...
    tx_hash: tx_hash || null,
    metadata: { tier, service: 'Hive Feature Flag', expires_ms: record.expires_ms },
  });
  ...
  return res.json({
    ...
    partner_attribution: 'Feature flag service — Spectral-signed rollback attestation. Complements LaunchDarkly, Unleash.',
    brand: '#C08D23',
